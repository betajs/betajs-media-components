    ...
    return Class.extend({
            ...
            return {
                ...
                attrs: function() {
                    return {
                        ...
                        "sidebaroptions": {
                            ...
                            "aspectratio": 1.778, // if set, then sidebar will be resized based on video container ratio, 16/9(=1.778), 4/3(1.333), 1/1, 9/16(0.5625), 3/4(0.75)
                ...
                computed: {
                    ...
                    // Will create/activate a new sidebar and shouldn't destroy it
                    // and will be only used inside `show_sidebar` computed listener
                    ...
                    // Will create/activate a new sidebar and shouldn't destroy it
                    // and will be only used inside `show_sidebar` computed listener
                    "hide_sidebar:playlist,is_floating,with_sidebar,showsidebargallery,states.sidebarascolumn,activatecompanionsidebar": function(
                        playlist,
                        isFloating,
                        withSidebar,
                        showSidebarGallery,
                        sidebarAsColumn,
                        activateCompanionSidebar
                    ) {
                        if (this.get("states.sidebarclosed") || (showSidebarGallery && !sidebarAsColumn)) {
                            if (showSidebarGallery && !sidebarAsColumn) {
                                console.warn(`Sidebar gallery will not be activated, as you has to set sidebar position, "left" or "right" (default)`);
                            }
                            return true;
                        }
                        if (!showSidebarGallery && Types.is_defined(this.get("initialoptions.nextwidget"))) {
                        ...
                        let showSidebar = (withSidebar && isFloating);
                        showSidebar ||= (showSidebarGallery && (playlist && playlist.length >= 0));
                        showSidebar ||= activateCompanionSidebar;
                        if (this.get("sidebar_active") !== true && showSidebar) {
                            this.set("sidebar_active", true);
                        }
                        // we can activate only once, after we should hide sidebar
                    ...
                    // Will handle show or hide sidebar based on conditions,
                    // and not re-created element and not re-render, only changes CSS display property
                    "show_sidebar:outstream,hide_sidebar,is_floating,with_sidebar,fullscreened,mobileviewport,showcompanionsidebar": function(
                        outstream,
                        hideSidebar,
                        isFloating,
                        withSidebar,
                        ...
                        this.set("gallerysidebar", !hideSidebar && this.__hideSidebarGalleryConditions(mobileViewport, isFloating, outstream, showCompanionSidebar));
                        this.set("companionsidebar", !hideSidebar && showCompanionSidebar);
                        ...
                        return this.get("floatingsidebar") || this.get("gallerysidebar") || this.get("companionsidebar");
                    },
                    "adsinitialized:playing,adtagurl,inlinevastxml": function(playing, adsTagURL, inlineVastXML) {
                        if (this.get("adsinitialized") || playing) {
                    ...
                    "containerSizingStyles:computedstyles,is_floating": function(styles, isFloating) {
                        ...
                        if (isFloating) {
                            ...
                            this.__lastActiveElementStyles = this._applyStyles(this.activeElement(), activeElement, null);
                            if (aspectRatio) {
                    ...
                    "cssfloatingclasses:floatingoptions.desktop.position,states.floatingclosed": function(position, closed) {
                        if (closed) return "";
                        return [
                ...
                initAdSources: function() {
                    ...
                    if (this.get("adshassource")) {
                        ...
                        const skipInitialWithoutAutoplay = this.get("skipinitial") && !this.get("autoplay");
                        ...
                        this.on("change:companionads", function(companionAds) {
                            if (this.__repeatOutstream && this.get("outstreamoptions.persistentcompanionad"))
                                return;
                            const companionAdsExist = companionAds && companionAds.length > 0;
                            this.set("activatecompanionsidebar", this.get("sidebaroptions.showcompanionad") && companionAdsExist);
                            if (companionAdsExist && this.get("companionad")) {
                                if (this.get("companionad.locations")) {
                                    this._renderMultiCompanionAds();
                                } else if (this.scopes.adsplayer && Types.is_string(this.get("companionad")) || Types.is_boolean(this.get("companionad"))) {
                                    this.scopes.adsplayer.execute('renderCompanionAd', this.get("ad"), this.get("companionad"));
                ...
                __initFloatingOptions: function() {
                    Objs.iter(["mobile", "desktop"], function(view) {
                        const _floatingoptions = this.get("floatingoptions")[view];
                        if (_floatingoptions && _floatingoptions.size && _floatingoptions.availablesizes) {
                            const _availableSizes = _floatingoptions.availablesizes;
                        ...
                        if (presetHeight) {
                            ...
                            if (expectedPlayerAspectRatioNum && !presetWidth) {
                                ...
                                if (
                                    ...
                                    this.get(`sidebaroptions.aspectratio`) !== null &&
                                    sidebarWidthNum > 0
                                ) {
                ...
                __containerSizeComputeHelperFunctions: function() {
                    ...
                    const centerPlayerIfSmallerThanContainer = (styles, playerWidthNum, containerWidthNum, moreThanPx) => {
                        moreThanPx = (!isNaN(moreThanPx) ? moreThanPx : 10) || 10;
                        if (playerWidthNum + moreThanPx < containerWidthNum) {
                            styles = {
                                height: `100%`,
                                ...styles,
                                margin: `0 auto`,
                ...
                __convertToCSSSupportedUnit: function(measure, propertyName) {
                    ...
                    measure = isNaN(measure) ? measure : parseFloat(measure).toFixed(2) + "px";
                    if (window && !window.CSS.supports(propertyName, measure)) {
                        let message = `Value: "${measure}" you provided as CSS unit, not supported by the browser`;
                        message += propertyNamePassed ? `for the CSS ${propertyName} property name;` : ``;
                ...
                // passing arguments only fo the reason, show dependency inside event listener, where this func calls
                __hideSidebarGalleryConditions: function(mobileViewPort, isFloating, outstream, showCompanionSidebar) {
                    mobileViewPort = mobileViewPort || this.__isInMobileViewport();
                    outstream = outstream || this.get("outstream");
                    showCompanionSidebar = showCompanionSidebar || this.get("showcompanionsidebar");
                    isFloating = isFloating || this.get("is_floating");
                    let showCondition = !isFloating;
                    showCondition &&= (Types.is_defined(mobileViewPort) && !mobileViewPort);
                    showCondition &&= !(outstream && showCompanionSidebar);
